...
class ItemsService {
  ...
  func fetchFullProduct(_ product: PassCityProduct, _ completion: @escaping ServiceCompletion<PassCityProduct>) {
    itemsProvider.callAPI(.getProduct(type: product.type.rawValue, id: product.id)) { result in
      switch result {
      case .success(let response):
  ...
  func fetchFullItem(_ product: PassCityFeedItem, _ completion: @escaping ServiceCompletion<PassCityFeedItem>) {
    itemsProvider.callAPI(.getProduct(type: product.type.rawValue, id: product.id)) { result in
      switch result {
      case .success(let response):
        guard let feedResponse = Mapper<PassCityFeedItem>().map(JSONObject: response.data) else {
          return completion(.failure(DataError.serializationError(response.data)))
        }
        return completion(.success(feedResponse))
      case .failure(let error):
